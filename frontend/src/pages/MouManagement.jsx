...
const MouManagement = () => {
  ...
  // Use another useEffect to save MOUs to local storage whenever `mous` changes
  useEffect(() => {
    try {
  ...
  }, [mous]); // Dependency array: run this effect whenever `mous` changes
  ...
  useEffect(() => {
    // Initialize activity log or load from local storage if needed
    // For now, keeping it as-is, but you might want to persist this too
    setActivityLog([
      "✅ MOU created between Riphah & TechNova (Jan 10, 2025)",
      "⚠️ MOU expired: Riphah × InnoSoft (Aug 1, 2024)",
    ]);
  ...
  const handleCreate = async (e) => {
    ...
    const newMou = {
      ...formData,
      id: mous.length + Math.floor(Math.random() * 1000000) + 1, // Ensure unique ID
    };
    setMous((s) => [newMou, ...s]); // This will trigger the useEffect to save to localStorage
    setActivityLog((a) => [
      `✅ MOU created between ${newMou.university} & ${newMou.industry} (Starts ${newMou.startDate})`,
      ...a,
    ]);
  ...
  const generatePDFWithComments = async (mou) => {
    ...
    page.drawText("--- Purpose & Scope ---", { x: leftMargin, y, font: boldFont, size: 14, color: headingColor });
    y -= 20;
    // Handle potential line breaks for long descriptions
    const descriptionText = mou.description || "—";
    const descriptionLines = descriptionText.match(/.{1,90}/g) || [descriptionText]; // Wrap text to 90 chars per line
    descriptionLines.forEach(line => {
      page.drawText(line, { x: leftMargin + 10, y, font, size: 12 });
      y -= lineHeight;
    });
    ...
    if (mou.extraDetails.filter(d => d.trim() !== "").length > 0) {
      ...
      mou.extraDetails.forEach(d => {
        if (d.trim()) {
          const detailLines = d.match(/.{1,90}/g) || [d]; // Wrap text to 90 chars per line
          detailLines.forEach(line => {
            page.drawText(`• ${line}`, { x: leftMargin + 10, y, font, size: 12, color: textColor });
            y -= lineHeight;
          });
        }
      });
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.rightCol}>
          ...
          >
            ...
            <div style={{ fontSize: 12, color: "#193648" }}>
              • Use search & filter to quickly find MOUs.<br />
              • Instantly download MOU PDFs for records.<br />
              • Create MOUs efficiently with auto-PDF generation.<br />
            </div>
          </motion.div>
